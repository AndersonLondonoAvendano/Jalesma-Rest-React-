...
function DetalleMaterial() {
  ...
  const handleEliminar = async () => {
    setEliminando(true);
    try {
      await api.delete(`/materiales/eliminar/${id}`);
      navigate("/materiales");
    } catch (error) {
  ...
  if (!material) {
    return (
      <div className="min-h-screen bg-jalesma-black flex items-center justify-center px-4">
        <div className="text-center">
          <h3 className="text-2xl text-jalesma-gold-light mb-4">No se encontró el material</h3>
          <Link 
            to="/materiales"
          ...
          <div className="p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-jalesma-gray-medium rounded-lg p-6 border border-jalesma-gold/10">
                ...
                <p className="text-jalesma-gold text-2xl font-bold">#{material.id}</p>
              </div>

              {/* Tipo */}
              <div className="bg-jalesma-gray-medium rounded-lg p-6 border border-jalesma-gold/10">
                <p className="text-jalesma-gold-light/60 text-sm mb-2 uppercase tracking-wider">Tipo</p>
                <p className="text-white text-2xl font-semibold">{material.tipo}</p>
              </div>

              {/* Precio */}
              <div className="bg-jalesma-gray-medium rounded-lg p-6 border border-jalesma-gold/10">
                <p className="text-jalesma-gold-light/60 text-sm mb-2 uppercase tracking-wider">Precio</p>
                <p className="text-jalesma-gold text-2xl font-bold">${material.precio}</p>
              </div>
              ...
              <div className="bg-jalesma-gray-medium rounded-lg p-6 border border-jalesma-gold/10">
                ...
                {material.bolsos.length > 0 ? (
                  <div className="flex flex-wrap gap-2 mt-2">
                    {material.bolsos.map((bolso, index) => (
                      <span 
                        key={index}
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 px-4">
          <div className="bg-jalesma-gray-dark rounded-lg shadow-2xl border-2 border-red-500/50 max-w-md w-full p-8 transform transition-all">
            ...
            <div className="text-center mb-6">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">¿Eliminar Material?</h3>
              <p className="text-jalesma-gold-light">
                Esta acción no se puede deshacer. El material <span className="text-jalesma-gold font-semibold">{material.tipo}</span> será eliminado permanentemente.
              </p>
            </div>
